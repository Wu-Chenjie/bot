...
class PoetryPlugin(NcatBotPlugin):
    ...
    async def poetry_cmd(self, event: BaseMessageEvent):
        ...
        if not poetry_text:
            await event.reply(MessageChain([Plain("获取诗歌失败，请稍后重试~")]))
            return
        await event.reply(MessageChain([Plain(poetry_text)]))
    ...
    async def poetry_type_cmd(self, event: BaseMessageEvent, type: str = "classic"):
        ...
        if type not in ["classic", "modern"]:
            await event.reply(MessageChain([Plain("类型错误！支持：classic(古诗词) / modern(现代诗)")]))
            return
        
        poetry_text = await PoetryAPI.get_poetry_by_type(type)
        if not poetry_text:
            await event.reply(MessageChain([Plain(f"获取{type}类型诗歌失败，请稍后重试~")]))
            return
        await event.reply(MessageChain([Plain(poetry_text)]))
    ...
    @param(name="group_id", type=int, help="群聊ID")
    async def poetry_schedule_cmd(self, event: BaseMessageEvent, action: str = "add", group_id: int = None):
        """配置定时发送群聊"""
        # 仅群聊管理员可操作
        if not isinstance(event, GroupMessageEvent):
            await event.reply(MessageChain([Plain("该命令仅支持群聊使用~")]))
            return
        
        # 未指定群聊则使用当前群聊
        target_group = group_id or event.group_id
        ...
        await event.reply(MessageChain([Plain(msg)]))
